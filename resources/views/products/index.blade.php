        ...
        <a href="{{ route('products.show', $product->slug) }}" class="card bg-white dark:bg-gray-800 rounded-xl overflow-hidden flex flex-col h-full group no-underline transition-all duration-300 transform hover:-translate-y-2 hover:shadow-xl">
            ...
            <div class="p-4 flex-1 flex flex-col">
                ...
                <div class="flex gap-2">
                    ...
                    <button class="flex-1 border-2 border-primary text-primary hover:bg-primary hover:text-white px-4 py-2 rounded-lg font-semibold transition add-to-cart-btn" 
                        data-product-id="{{ $product->id }}" 
                        data-has-variants="{{ $product->hasVariants() ? 'true' : 'false' }}" 
                        data-product-slug="{{ $product->slug }}" 
                        @if($product->hasVariants())
                        data-requires-redirect="true"
                        @endif
...
function handleWishlistClick(e) {
    // Stop event from bubbling up to the card link
    e.stopPropagation();
...
function handleCartClick(e) {
    ...
    // Check if product requires redirect to product page (for variant selection)
    const requiresRedirect = this.hasAttribute('data-requires-redirect');
    const productSlug = this.getAttribute('data-product-slug');
    
    // If product requires redirect, go to product page instead of adding to cart
    if (requiresRedirect && productSlug) {
